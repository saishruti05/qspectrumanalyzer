...
class PowerThread(BasePowerThread):
    ...
    def parse_output(self, data):
        ...
        x_axis = np.linspace(start_freq, stop_freq,len(y_axis))# round((stop_freq - start_freq) / step))
    ...
    def run(self):
        ...
        # '''
        #create fake data
        header = namedtuple('header', 'time_start time_stop start stop step samples')
        header.time_start = 0
        header.time_stop = header.time_start + 10
        header.start = 100e6
        header.stop = 200e6
        header.step = 1e-1
        header.samples = 100
